...
class CnMaestroClient:
        ...
        while True:
            try:
                ...
                if resp.status_code != 200:
                    logger.warning(f"Stats fetch failed at offset {offset}: {resp.status_code}")
                    break
                ...
                for d in batch:
                    ...
                    # Extract Link Info
                    radio = d.get('radio', {})
                    ap_mac = d.get('ap_mac') or radio.get('ap_mac') or radio.get('parent_mac')
    ...
    def get_full_inventory(self, force_refresh: bool = False) -> Dict:
        ...
        """
        if self.inventory_cache and not force_refresh and (time.time() - self.last_cache_update < self.CACHE_DURATION):
        ...
        for d in all_devices:
            ...
            is_ap = mode in ['ap', 'master'] or name.upper().startswith('AP') or name.upper().startswith('BHS')
            ...
            if is_ap:
                ...
                if mac: ap_lookup[mac] = device_entry
            else:
                # Is SM
                # Store extra info for linking
                device_entry['ap_mac'] = stat.get('ap_mac')
                device_entry['tower_ref'] = structure[network][tower] # Ref to tower obj
